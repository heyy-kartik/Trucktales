...
export interface DriverMapProps {
  ...
  onReportSubmit?: (
    condition: Omit<RoadCondition, "id" | "timestamp" | "reportedBy">
  ) => void;
...
const DriverMap = forwardRef<DriverMapRef, DriverMapProps>(({ 
  ...
  const { isLoaded } = useLoadScript({
    ...
    const [currentLocation, setCurrentLocation] =
      useState<google.maps.LatLngLiteral | null>(propCurrentLocation || null);
    const [roadConditions, setRoadConditions] =
      useState<RoadCondition[]>(propRoadConditions);
    const [showReportDialog, setShowReportDialog] = useState(false);
    const [selectedCondition, setSelectedCondition] = useState<
      "accident" | "road_closure" | "bad_road" | "traffic"
    >("bad_road");
    ...
    const handleStationClick = useCallback(
      (station: FuelStation, e: google.maps.MapMouseEvent) => {
        e.stop();
    ...
    return (
      <div className="relative w-full h-full">
        ...
        >
          ...
          {/* Show fuel stations */}
          {activeView === "fuel" &&
            fuelStations?.map((station: FuelStation) => (
              <Marker
